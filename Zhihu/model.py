...
class Topic2Essay(torch.nn.Module):
    def __init__(self, gpt2_model_path, vocab_size, topic_vocab_size, kw_vocab_size=2430):
        ...
        self.gate = torch.nn.Linear(768 * 2, 1)
    ...
    def encode(self, src, src_mask):
        src_embed = self.topic_looking_table(src)
        src_mask = src_mask.unsqueeze(-2)
        neibor_embedding, neibor_mask = self.plugin_transformer.gat_base.get_neibor_embed(src)
        memory = self.plugin_transformer.model.encode(src, src_embed, src_mask, neibor_embedding, neibor_mask)
        return memory
